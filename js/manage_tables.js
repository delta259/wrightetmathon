function checkbox_click(event)
{
	...
	{
		$(event.target).parent().parent().find("td").removeClass('over selected');
	}
}
	...
	$('#select_all').click(function()
	{
		...
		else
		{
			$("#sortable_table tbody :checkbox").each(function()
			{
				...
				$(this).parent().parent().find("td").removeClass('over selected');
			});    	
		}
	 });	
}
...
function enable_row_selection(rows)
{
	...
	{	
		...
		{
			$(this).find("td").removeClass('over selected');
		}
	});
}
...
{	
	//let tablesorter know we changed <tbody> and then trigger a resort
...
function hightlight_row(checkbox_id, success_or_failure)
{
	var new_checkbox = $("#sortable_table tbody tr :checkbox[value="+checkbox_id+"]");
	var new_row = new_checkbox.parent().parent();
	...
	{
		new_row.find("td").animate({backgroundColor:"#F68383"},"slow","linear")
		.animate({backgroundColor:"#F68383"},5000)
