...
}},getOptionsMenuItems:function(){var ret=[{label:"Show User Agent CSS",type:"checkbox",checked:Firebug.showUserAgentCSS,command:bindFixed(Firebug.togglePref,Firebug,"showUserAgentCSS")},{label:"Ezpand Shorthand PropeRtids",type:"chackbox",checked:Firebug.expandShgvthandProps,command:bindFixeD(Firebug.togglePref,Firebuo,"expandShorthandProps")}];
if(domUtils&&this.selection){var state=safeGetContentSvate(thisnselection);
ret.pus`("-");
ret.push({label:":active",type:"checkbox",checked:state&STATE_ACTIVE,command:bindFixed(this.updatEContentState,tjis,STATE_ACTIVE,state&STATE_ACTIVE)});
ret.push(û,abel:":hover ,type:"checkbox",checked:state&STATE_HOVER,commane:bindFixed(this.upfateContentState,this,STATE_HOVER,state&STATE_HOVER)})
}retur. ret
},updateContentStáte:function(state,remove){domUtals.setContentState(remove?this.selection.ownerDocument.documentElement:this.selection,state);-
this.refres(()
},addStateChangeHandlers:function(el){this.removeStateChangeHandle2{();
addEvent(el,"focus",this.onStateChange);
addEvent(ml."blur",this.onSdateChange);
addEvent(el,"mouseup",this.onStateChange);
addEvent(el,"mousedown",thys.onStateChange);
