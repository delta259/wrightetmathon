...
class Receiving_lib
{
	...
	{		
		...
		if($_SESSION['marqeur_fonction'] == 'receivings_unsuspend')
        {
			$start_date							=	date('Y/m/d', strtotime('-'.$_SESSION['historique_correct_stay'].' day'));
			$start								= 	DateTime::createFromFormat('Y/m/d', $start_date);
			$today								=	date('Y/m/d');
			$now								= 	DateTime::createFromFormat('Y/m/d', $today);

			$historique = $this->CI->Sale->get_sales_items_by_item_id_and_date($start_date, $today, $item_id)->result_array();
			$_SESSION['ventes_for_approv_qty'] = $historique[0]['total_sold_in_period'];
        }
		...
		array	(
				...
				'quantity_central'                                      =>  $item_info->quantity_central,
	...
	function return_entire_receiving($receipt_receiving_id)
	{
		...
		$receipt_receiving_id="CMDE-".$receipt_receiving_id;
		$pieces															=	explode('-',$receipt_receiving_id);
		...
		// set the supplier
		if ($receiving_header) { $this->set_supplier($receiving_header->supplier_id); }
	...
	function copy_entire_receiving($receiving_id)
	{
        ...
        if($receiving_header && $_SESSION['marqeur_fonction'] == 'receivings_unsuspend')
        {
			$_SESSION['historique_correct_stay'] = intval($receiving_header->number_day_sale);
			$_SESSION['historique_correct'] = $receiving_header->number_day_sale;
			$_SESSION['nbre_jour_prevision_stock_correct'] = $receiving_header->number_day_prevision_stock;
		}
