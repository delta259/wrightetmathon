...
class Cashtills extends CI_Controller
{
	...
	function open()
	{
		...
		// ALWAYS load all currency definitions
		$_SESSION['transaction_info']									=	$this->Currency_definition->get_all()->result();

		// determine if we have saved quantities from a previous submission
		$has_saved = isset($_SESSION['transaction_info_save']) && is_array($_SESSION['transaction_info_save']);

		// set quantities and fix multiplier where needed
		foreach ($_SESSION['transaction_info'] as $key => $row)
		{
			// use denomination value as multiplier if multiplier is 0
			if (empty($row->multiplier) || $row->multiplier == 0)
			{
				$row->multiplier										=	floatval($row->denomination);
			}
			// restore saved quantity if second pass, otherwise zero
			$row->quantity												=	($has_saved && isset($_SESSION['transaction_info_save'][$key]))
																			? intval($_SESSION['transaction_info_save'][$key])
																			: 0;
			$row->total													=	$row->quantity * $row->multiplier;
			$_SESSION['transaction_info'][$key] 						=	$row;
		}

		// set output defaults (first pass)
		...
		{
			$_SESSION['total_caisse_open']								+=	$row->total;
		}
	...
	function change()
	{
		...
		// load theoretical amounts for display
		$today															=	date("Y-m-d");
		$cash_sales_row													=	$this->Sale->get_cash_sales_by_date(array('start_date'=>$today, 'end_date'=>$today))->row();
		$_SESSION['change_cash_sales_today']							=	(float)($cash_sales_row->cash_total_today ?? 0);
		$open_row														=	$this->Cashtill->get(date("Y"), date("m"), date("d"), 'OPEN')->row();
		$_SESSION['change_open_amount']									=	(float)($open_row->cash_amount ?? 0);
		$_SESSION['change_theoretical_total']							=	$_SESSION['change_open_amount'] + $_SESSION['change_cash_sales_today'];
	...
	{		
		...
		// load bank deposit history for current year
		$_SESSION['bank_deposit_history'] = $this->Cashtill->get_bank_deposits_year(date("Y"))->result();
	...
	{
		$year  = $this->input->post('hist_year')  ? intval($this->input->post('hist_year'))  : intval($_SESSION['bank_hist_year'] ?? date('Y'));
		$month = $this->input->post('hist_month') ? intval($this->input->post('hist_month')) : intval($_SESSION['bank_hist_month'] ?? 0);

		$_SESSION['show_dialog']       = 7;
		$_SESSION['bank_hist_year']    = $year;
		$_SESSION['bank_hist_month']   = $month;
		$_SESSION['bank_deposit_history'] = $this->Cashtill->get_bank_deposits_year($year, $month)->result();
	...
	function mouvements()
	{
		$year  = $this->input->post('mvt_year')  ? intval($this->input->post('mvt_year'))  : intval($_SESSION['mvt_year'] ?? date('Y'));
		$month = $this->input->post('mvt_month') !== FALSE ? intval($this->input->post('mvt_month')) : intval($_SESSION['mvt_month'] ?? 0);

		$_SESSION['show_dialog'] = 8;
		$_SESSION['mvt_year']    = $year;
		$_SESSION['mvt_month']   = $month;

		if ($month > 0) {
			// Daily detail for a given month
			$rows = $this->Cashtill->get_movements_by_day($year, $month)->result();
			$_SESSION['mvt_mode'] = 'daily';
		} else {
			// Monthly summary for the whole year
			$rows = $this->Cashtill->get_movements_monthly($year)->result();
			$_SESSION['mvt_mode'] = 'monthly';
		}
		...
		if ($month > 0) {
			// Daily: create entries for each day that has data
			$days_data = [];
			foreach ($rows as $r) {
				$key = intval($r->cash_day);
				if (!isset($days_data[$key])) {
					$days_data[$key] = array('CASH_SALES' => 0, 'SET_ASIDE' => 0, 'BANK_DEPOSIT' => 0);
				}
				$days_data[$key][$r->cash_code] = round((float)$r->total_amount, 2);
			}
			ksort($days_data);
			foreach ($days_data as $day => $vals) {
				$labels[]   = str_pad($day, 2, '0', STR_PAD_LEFT);
				$sales[]    = $vals['CASH_SALES'];
				$setaside[] = $vals['SET_ASIDE'];
				$bank[]     = $vals['BANK_DEPOSIT'];
			}
		} else {
			// Monthly: 12 months
			$months_data = [];
			foreach ($rows as $r) {
				$key = intval($r->cash_month);
				if (!isset($months_data[$key])) {
					$months_data[$key] = array('CASH_SALES' => 0, 'SET_ASIDE' => 0, 'BANK_DEPOSIT' => 0);
				}
				$months_data[$key][$r->cash_code] = round((float)$r->total_amount, 2);
			}
			$month_names = array(1=>'Jan',2=>'Fév',3=>'Mar',4=>'Avr',5=>'Mai',6=>'Jun',7=>'Jul',8=>'Aoû',9=>'Sep',10=>'Oct',11=>'Nov',12=>'Déc');
			for ($m = 1; $m <= 12; $m++) {
				$labels[] = $month_names[$m];
				$sales[]    = isset($months_data[$m]) ? $months_data[$m]['CASH_SALES'] : 0;
				$setaside[] = isset($months_data[$m]) ? $months_data[$m]['SET_ASIDE'] : 0;
				$bank[]     = isset($months_data[$m]) ? $months_data[$m]['BANK_DEPOSIT'] : 0;
			}
		}

		$_SESSION['mvt_labels']   = $labels;
		$_SESSION['mvt_sales']    = $sales;
		$_SESSION['mvt_setaside'] = $setaside;
		$_SESSION['mvt_bank']     = $bank;
	...
	function close()
	{
		...
		$today															=	date("Y-m-d");
		$cash_sales_row													=	$this->Sale->get_cash_sales_by_date(array('start_date'=>$today, 'end_date'=>$today))->row();
		$_SESSION['close_cash_sales_today']								=	(float)($cash_sales_row->cash_total_today ?? 0);
		$open_row														=	$this->Cashtill->get(date("Y"), date("m"), date("d"), 'OPEN')->row();
		$_SESSION['close_open_amount']									=	(float)($open_row->cash_amount ?? 0);
		$_SESSION['close_theoretical_total']							=	$_SESSION['close_open_amount'] + $_SESSION['close_cash_sales_today'];
	...
	function send_report_mail()
	{
		...
		$data_customer_open = $this->Employee->get_info($_SESSION['cash_transaction_open'][0]['person_id']);	
		$data_customer_close = $this->Employee->get_info($_SESSION['cash_transaction_close'][0]['person_id']);		
		
		$msg_sent = $this->lang->line('cashtills_open')	." :\t".$_SESSION['cash_transaction_open'][0]['timestamp']." : ".$_SESSION['cash_transaction_open'][0]['cash_amount']."\t".$data_customer_open->first_name." ".$data_customer_open->last_name."\r\n"
					.$this->lang->line('cashtills_closed')	." :\t\t".$_SESSION['cash_transaction_close'][0]['timestamp']." : ".$_SESSION['cash_transaction_close'][0]['cash_amount'] ."\t".$data_customer_close->first_name." ".$data_customer_close->last_name. " " ."\r";
		$msg_sent_line="\r\n";
