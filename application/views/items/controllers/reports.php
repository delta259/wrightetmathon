...
{	
	...
	// this input applies to tabular and graphical reports.
	...
	{				
		...
		if ($transaction_type == 'SR')
		{
			...
			$data['summary_data']['average_basket']						=	$data['summary_data']['subtotal'] / $data['summary_data']['invoice_count'];
		}
	...
	{  
       ...
       // clear data arrays
		...
		// get the report data 
		$report_data 													=	$this->$model_name->getDataz(array('start_date'=>$start_date, 'end_date'=>$end_date, 'transaction_subtype' => $transaction_type));
		...
		{
			// get offered count
			$data['summary_data']										+=	$this->$model_name->offered_countz(array('start_date'=>$start_date, 'end_date'=>$end_date, 'transaction_subtype' => $transaction_subtype));
...
//uniquement pour le ticket_Z modif 
...
{		
	...
	{		
		...
		$_SESSION['report_data'] 										= 	$this->Stock_queries->getData($inputs);
	...
	function inventory_slowmoving($export_excel=0, $create_PO=0, $set_NM=0, $set_SM=0, $month_end=0)
	{
		...
		if ($set_SM == 1)
		{
			$subtitle = date($this->config->item('dateformat'), strtotime($start_date)).' - '.date($this->config->item('dateformat'), strtotime($end_date)).' '.$this->lang->line('common_for').' '.$this->db->database.'. '."<font color='red'>".$this->lang->line('reports_set_reorder_policy_S')."</font><br>";
		}
		else
		{
			$subtitle = date($this->config->item('dateformat'), strtotime($start_date)).' - '.date($this->config->item('dateformat'), strtotime($end_date)).' '.$this->lang->line('common_for').' '.$this->db->database.'.';
		}
	...
	function top_items_by_value()
	{
		...
		$data['links']													=	$this->pagination->create_links();
		$data['controller_name']										=	strtolower(get_class($this));
		$data['form_width']												=	$this->items->get_form_width();

		$create_headers													=	1;
		$_SESSION['order_by']											=	'sales_ht';
		$_SESSION['sequence']											=	'desc';
		
		$data['manage_table']		=	get_items_manage_table($this->Item->get_all($config['per_page'], $this->uri->segment($config['uri_segment'])), $this, $create_headers);
	...
	*/
	...
	function	update_stock_valuation_records ()
	{
		...
		if ($this->config->item('createstockvaluationrecords') == 'Y')
		{
			// load model
			$this->load->model('Item');
			...
			$item_data						=	$this->Item->get_all($where_select)->result_array();
