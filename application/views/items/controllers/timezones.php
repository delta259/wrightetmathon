...
class timezones extends CI_Controller
{
	function index()
	{
		...
		$create_headers													=	1;
		$data['manage_table']											=	get_timezones_manage_table($this->Timezone->get_all($config['per_page'], $this->uri->segment( $config['uri_segment'] ) ), $this, $create_headers);
	...
	function search()
	{
		...
		$create_headers													=	0;
		$data_rows														=	get_timezones_manage_table($this->Timezone->search($search), $this, $create_headers);
		echo $data_rows;
	}
	...
	*/
	...
	function verify()
	{
		...
		if ($_SESSION['new'] == 1 OR $_SESSION['transaction_info']->timezone_name != $_SESSION['original_timezone_name'])
		{
