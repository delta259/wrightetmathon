<?php $this->load->view("partial/header_popup"); ?>

<div open class="fenetre modale modal-md">
    <!-- Header -->
    <div class="fenetre-header">
        <span class="fenetre-title">
            <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <circle cx="12" cy="12" r="10"/><path d="M12 6v12m-4-4h8"/>
            </svg>
            <?php echo $_SESSION['$title'] . ' ' . $this->lang->line('modules_currencies'); ?>
        </span>
        <?php include('../wrightetmathon/application/views/partial/show_exit.php'); ?>
    </div>

    <!-- Content -->
    <div class="fenetre-content">
        <?php include('../wrightetmathon/application/views/partial/show_messages.php'); ?>

        <?php if (($_SESSION['del'] ?? 0) != 1): ?>

            <?php if (($_SESSION['new'] ?? 0) != 1 && ($_SESSION['undel'] ?? 0) != 1): ?>
                <div style="text-align: right; margin-bottom: 16px;">
                    <?php
                    echo form_open('currencies/delete/'.$_SESSION['transaction_id'], array('id'=>'currency_delete_form', 'style'=>'display:inline'));
                    echo form_submit(array(
                        'name' => 'delete',
                        'id' => 'delete',
                        'value' => $this->lang->line('currencies_delete'),
                        'class' => 'btmodification'
                    ));
                    echo form_close();
                    ?>
                </div>
            <?php endif; ?>

            <?php if (($_SESSION['undel'] ?? 0) != 1):
                echo form_open($_SESSION['controller_name'].'/save/', array('id'=>'item_form'));
            ?>

            <fieldset class="fieldset">
                <legend><?php echo $this->lang->line('currencies_currency_info'); ?></legend>

                <div class="form-grid">
                    <!-- Row 1: Name & Description -->
                    <div class="form-row">
                        <div class="form-group" style="flex: 0 0 150px;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_name'); ?></label>
                            <?php echo form_input(array(
                                'name' => 'currency_name',
                                'id' => 'currency_name',
                                'class' => 'form-control',
                                'placeholder' => 'EUR, USD...',
                                'value' => $_SESSION['transaction_info']->currency_name,
                                'maxlength' => 15
                            )); ?>
                        </div>
                        <div class="form-group" style="flex: 1;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_description'); ?></label>
                            <?php echo form_input(array(
                                'name' => 'currency_description',
                                'id' => 'currency_description',
                                'class' => 'form-control',
                                'placeholder' => 'Euro, Dollar...',
                                'value' => $_SESSION['transaction_info']->currency_description
                            )); ?>
                        </div>
                    </div>

                    <!-- Row 2: Code, Sign & Display Order -->
                    <div class="form-row">
                        <div class="form-group" style="flex: 0 0 100px;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_code'); ?></label>
                            <?php echo form_input(array(
                                'name' => 'currency_code',
                                'id' => 'currency_code',
                                'class' => 'form-control',
                                'placeholder' => 'EUR',
                                'value' => $_SESSION['transaction_info']->currency_code,
                                'maxlength' => 10
                            )); ?>
                        </div>
                        <div class="form-group" style="flex: 0 0 80px;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_sign'); ?></label>
                            <?php echo form_input(array(
                                'name' => 'currency_sign',
                                'id' => 'currency_sign',
                                'class' => 'form-control',
                                'placeholder' => '€',
                                'value' => $_SESSION['transaction_info']->currency_sign,
                                'maxlength' => 10
                            )); ?>
                        </div>
                        <div class="form-group" style="flex: 0 0 100px;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_display_order'); ?></label>
                            <?php echo form_input(array(
                                'name' => 'currency_display_order',
                                'id' => 'currency_display_order',
                                'type' => 'number',
                                'class' => 'form-control',
                                'placeholder' => '1',
                                'value' => $_SESSION['transaction_info']->currency_display_order
                            )); ?>
                        </div>
                    </div>

                    <!-- Row 3: Side & Tax -->
                    <div class="form-row">
                        <div class="form-group" style="flex: 1;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_side'); ?></label>
                            <?php echo form_dropdown(
                                'currency_side',
                                $_SESSION['LorR_pick_list'],
                                $_SESSION['transaction_info']->currency_side,
                                'class="form-control"'
                            ); ?>
                        </div>
                        <div class="form-group" style="flex: 1;">
                            <label class="form-label required"><?php echo $this->lang->line('currencies_currency_tax'); ?></label>
                            <?php echo form_dropdown(
                                'currency_tax',
                                $_SESSION['G']->YorN_pick_list,
                                $_SESSION['transaction_info']->currency_tax,
                                'class="form-control"'
                            ); ?>
                        </div>
                    </div>
                </div>
            </fieldset>

            <div id="required_fields_message" class="obligatoire">
                <?php echo $this->lang->line('common_fields_required_message'); ?>
            </div>

            <div class="txt_milieu">
                <?php echo anchor('common_controller/common_exit/', '<div class="btretour btlien">'.$this->lang->line('common_reset').'</div>', 'target="_self"'); ?>
                <?php echo form_submit(array(
                    'name' => 'submit',
                    'id' => 'submit',
                    'value' => $this->lang->line('common_submit'),
                    'class' => 'btsubmit'
                )); ?>
            </div>

            <?php echo form_close(); ?>
            <?php endif; ?>
        <?php endif; ?>
    </div>
</div>

<style>
.form-grid { display: flex; flex-direction: column; gap: 16px; }
.form-row { display: flex; gap: 16px; flex-wrap: wrap; }
.form-group { display: flex; flex-direction: column; gap: 6px; min-width: 0; }
.form-label { font-size: 0.85rem; font-weight: 500; color: var(--text-secondary, #64748b); }
.form-label.required::after { content: ' *'; color: var(--danger, #ef4444); }
.form-control {
    padding: 10px 12px;
    border: 1px solid var(--border-color, #e2e8f0);
    border-radius: 6px;
    font-size: 0.9rem;
    background: var(--bg-input, #fff);
    color: var(--text-primary, #1e293b);
    transition: border-color 0.2s, box-shadow 0.2s;
}
.form-control:focus {
    outline: none;
    border-color: var(--primary, #0A6184);
    box-shadow: 0 0 0 3px rgba(10, 97, 132, 0.15);
}
</style>
