...
<div class="table-container">
    <div class="table-wrapper">
        <?php
        $countries = $country_data ?? null;
        $has_rows = ($countries && $countries->num_rows() > 0);
        ?>
        <?php if ($has_rows): ?>
        <table class="tablesorter" id="sortable_table">
            <thead>
                <tr>
                    <th><?php echo $this->lang->line('countries_country_name'); ?></th>
                    <th><?php echo $this->lang->line('countries_country_description'); ?></th>
                    <th><?php echo $this->lang->line('countries_country_display_order'); ?></th>
                    <th>Actions</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($countries->result() as $country): ?>
                <tr class="country-row" data-href="<?php echo site_url('countries/view/' . $country->country_id); ?>" style="cursor:pointer;">
                    <td><?php echo anchor('countries/view/' . $country->country_id, htmlspecialchars($country->country_name)); ?></td>
                    <td><?php echo htmlspecialchars($country->country_description); ?></td>
                    <td style="text-align:center;"><?php echo htmlspecialchars($country->country_display_order); ?></td>
                    <td style="text-align:center;white-space:nowrap;">
                        <a href="#" onclick="if(confirm('Supprimer ce pays ?')){window.location='<?php echo site_url('countries/delete/' . $country->country_id); ?>';} return false;" title="Supprimer" style="text-decoration:none;">
...
$(document).ready(function() {
    ...
    $('#sortable_table').on('click', 'tr.country-row td:not(:last-child)', function() {
...
{
    include('../wrightetmathon/application/views/countries/form.php');
