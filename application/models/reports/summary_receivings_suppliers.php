...
class Summary_receivings_suppliers extends Report
{
	...
	{
		return array($this->lang->line('reports_supplier'), $this->lang->line('reports_subtotal'));
	}
	
	public function getData(array $inputs)
	{
		$this->db->select('CONCAT(last_name, " ",first_name) as supplier, sum(quantity_purchased * item_cost_price) as subtotal', false);
		$this->db->from('receivings');
		$this->db->join('receivings_items', 'receivings.receiving_id = receivings_items.receiving_id');
		$this->db->join('people', 'receivings.supplier_id = people.person_id', 'left');
