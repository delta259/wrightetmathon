...
class Item_taxes extends CI_Model
{
	...
	*/
	function	get_info			($item_id)
	{
		...
		$this						->	db->where('items_taxes.branch_code', $this->config->item('branch_code'));
	...
	*/
	function	save				($items_taxes_data)
	{
		//Run these queries as a transaction, we want to make sure we do all or nothing
		$this						->	db->trans_start();
		
			// delete existing records
			$this						->	delete($_SESSION['transaction_info']->item_id);
			
			// add record
			$this						->	db->insert('items_taxes', $items_taxes_data);		
	...
	*/
	...
	{
		return 						$this->db->delete	('items_taxes',	array	(
																				'item_id' => $item_id,
