...
class Giftcard extends CI_Model
{
	...
	*/
	...
	function exists_by_number($giftcard_number)
	{
		$this															->	db->from('giftcards');
	...
	*/
	...
	{
		$this->db->from('giftcards');
		$this->db->join('people','people.person_id=giftcards.customer_id');//GARRISON ADDED 4/25/2013
		$this->db->where('deleted',0);
		$this						->	db->where('giftcards.branch_code', $this->config->item('branch_code'));
	...
	*/
	...
	function get_info_by_number($giftcard_number)
	{
		$this->db->from('giftcards');
		$this->db->where('giftcard_number', $giftcard_number);
		$this->db->where('giftcards.branch_code', $this->config->item('branch_code'));
...
/** GARRISON MODIFIED 4/24/2013 **/	
	...
	*/
	...
	function update_giftcard_value( $giftcard_number, $value_used )
	{
		$this->db->where('giftcard_number', $giftcard_number);
		$this->db->where('giftcards.branch_code', $this->config->item('branch_code'));
		$this->db->update('giftcards', array('value_used' => $value_used));
	}
	
	function	merge_customer		($merge_from_client, $update_data)
	{
		$this						->	db->where('giftcards.customer_id', $merge_from_client);
		$this						->	db->where('giftcards.branch_code', $this->config->item('branch_code'));
		$success					=	$this->db->update('giftcards', $update_data);
